...
function generateEllipse({ longRadio, shortRadio, gap = 1 }) {
    ...
    for (let index = 0; index < 360; index = index + gap) {
        ...
        const x = Math.cos(radian) * longRadio;
        const y = Math.sin(radian) * shortRadio;
        data.push({ x, y });
    }
    return data;
}
...
function rotateEllipse(rotateAngle, ellipseVec) {
    ...
    for (let i = 0; i < ellipseVec.length; i++) {
        ...
        point.x = rawX * Math.cos(radian) - rawY * Math.sin(radian);
        point.y = rawX * Math.sin(radian) + rawY * Math.cos(radian);
    }
}
